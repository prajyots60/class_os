...
describe('Role → Capability Resolver Engine Suite', () => {
  ...
  describe('Explicit Role Capability Sets (Matrix Tests)', () => {
    it('Test 1 — Owner capabilities (All 49 capabilities)', () => {
    ...
    it('Test 2 — Teacher capabilities (Exact 27 capabilities)', () => {
      const teacherCaps = getCapabilitiesForRole('teacher');
      expect(teacherCaps.size).toBe(27);

      // Teacher possesses academic, attendance, homework, test, marks, announcement, institute:read
    ...
    it('Test 3 — Assistant capabilities (Exact 19 capabilities)', () => {
      const assistantCaps = getCapabilitiesForRole('assistant');
      expect(assistantCaps.size).toBe(19);

      // Assistant possesses staff:read, student:create, payment:record, receipt:issue
      ...
      // Assistant MUST NOT possess marks publishing or staff role change
    ...
    it('Test 4 — Parent capabilities (Exact 10 capabilities)', () => {
      ...
      expect(parentCaps.size).toBe(10);
