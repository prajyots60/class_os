/**
 * POST /api/v1/communication/announcements/[id]/archive — Archive published announcement
 * GET    — 405
 * PUT    — 405
 * PATCH  — 405
 * DELETE — 405
...
export async function POST(req: NextRequest, { params }: RouteContext) {
  ...
  return withV1MutationGuard(req, requestId, async (ctx) => {
    ...
    const useCase = new ArchiveAnnouncementUseCase(repo);

    const archived = await useCase.execute(ctx, id);
    return apiSuccess(archived, requestId);
  });
}
