...
export async function GET(req: NextRequest) {
  ...
  return withV1ReadGuard(req, requestId, async (ctx) => {
    if (ctx.role === 'parent') {
      throw new AuthorizationError('Parent role is not authorized to access staff operational reports.');
    }
    ...
    const useCase = new GetAttendanceReportUseCase(repository);
    ...
    const dto = await useCase.execute({
      ...
      subjectId,
      teacherId,
      teacherIdScope: ctx.role === 'teacher' ? ctx.userId : undefined,
      search,
      page,
      pageSize,
      sortBy,
