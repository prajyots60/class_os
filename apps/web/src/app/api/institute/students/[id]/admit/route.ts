  ...
  try {
    ...
    if (contentType && contentType.includes('application/json')) {
      try {
        ...
        const parseResult = admitStudentSchema.safeParse(rawBody);
        if (!parseResult.success) {
          throw new ValidationError(
            'Invalid student admit payload.',
            parseResult.error.flatten().fieldErrors,
          );
        }
        admissionDate = parseResult.data.admissionDate;
      } catch (err) {
    ...
    const useCase = new AdmitStudentUseCase(studentRepo);

    const admitted = await useCase.execute(tenantContext, { id, admissionDate });
