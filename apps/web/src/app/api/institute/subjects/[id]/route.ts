...
export async function GET(
  ...
  try {
    ...
    const useCase = new GetSubjectUseCase(subjectRepo);

    const subject = await useCase.execute(tenantContext, { id });
...
export async function PATCH(
  ...
  try {
    ...
    const parseResult = updateSubjectSchema.safeParse(rawBody);
    if (!parseResult.success) {
      const fieldErrors = parseResult.error.flatten().fieldErrors;
      throw new ValidationError('Invalid subject update payload.', fieldErrors);
    }

    const subjectRepo = new PrismaSubjectRepository();
    const useCase = new UpdateSubjectUseCase(subjectRepo);
