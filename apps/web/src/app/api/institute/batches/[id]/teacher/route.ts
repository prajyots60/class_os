...
export async function POST(
  ...
  try {
    ...
    const parseResult = assignBatchTeacherSchema.safeParse(rawBody);
    if (!parseResult.success) {
      const fieldErrors = parseResult.error.flatten().fieldErrors;
      throw new ValidationError('Invalid assign teacher payload.', fieldErrors);
    }

    const batchRepo = new PrismaBatchRepository();
    const membershipRepo = new PrismaInstituteMembershipRepository();
    const useCase = new AssignBatchTeacherUseCase(batchRepo, membershipRepo);
