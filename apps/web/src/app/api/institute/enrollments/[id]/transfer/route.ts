...
export async function POST(
  ...
  try {
    ...
    const parseResult = transferEnrollmentSchema.safeParse(rawBody);
    if (!parseResult.success) {
      const fieldErrors = parseResult.error.flatten().fieldErrors;
      throw new ValidationError('Invalid transfer enrollment payload.', fieldErrors);
    }

    const enrollmentRepo = new PrismaEnrollmentRepository();
    const batchRepo = new PrismaBatchRepository();
    const useCase = new TransferEnrollmentUseCase(enrollmentRepo, batchRepo);

    const transferred = await useCase.execute(tenantContext, {
      id,
      targetBatchId: parseResult.data.targetBatchId,
    });
