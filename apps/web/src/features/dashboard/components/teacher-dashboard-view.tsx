...
import { TeacherPendingHomework } from './teacher-pending-homework';
import { TeacherUpcomingTests } from './teacher-upcoming-tests';
import { Card, CardHeader, CardTitle, CardDescription, CardFooter, Button, Skeleton, Badge } from '@coaching-os/ui';
import { AlertTriangle, RefreshCw, GraduationCap, Calendar, Globe } from 'lucide-react';

export function TeacherDashboardView() {
  const { data, isLoading, isError, error, refetch, isRefetching } = useTeacherDashboard();

  // 1. Loading Experience (Preserves layout shape with Skeletons)
  if (isLoading) {
    return (
      <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6" data-testid="teacher-dashboard-loading">
        {/* Header Skeleton */}
        ...
        {/* Operational Attention Grid Skeleton */}
  ...
  if (isError || !data) {
    const userSafeMessage = error?.message && !error.message.includes('Prisma') && !error.message.includes('SQL')
      ? error.message
      : 'Unable to load Teacher Dashboard operational data. Please verify network connectivity and try again.';

    return (
      <div className="max-w-7xl mx-auto p-4 sm:p-6" data-testid="teacher-dashboard-error">
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6" data-testid="teacher-dashboard-view">
      {/* Header Landmark */}
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-[hsl(var(--border))] pb-4">
        <div>
          ...
          <p className="text-xs text-[hsl(var(--muted-foreground))] mt-0.5">
            Today&apos;s assigned sessions &amp; academic tasks
          </p>
        </div>
      ...
      <main className="space-y-6">
        {/* Section 1: Today's Classes & Sessions (Frozen Today's Batches Scope) */}
        <section aria-label="Today's Classes and Sessions">
          <TeacherTodaySessions sessions={data.todaySessions} />
        </section>

        {/* Section 2: Operational Attention (Homework & Upcoming Tests) */}
        <section className="grid gap-6 md:grid-cols-2" aria-label="Academic Attention and Upcoming Tests">
          <TeacherPendingHomework homework={data.pendingHomework} />
          <TeacherUpcomingTests tests={data.upcomingTests} />
        </section>
      </main>
    </div>
  );
