 ...
 *
 * Client Component — interactive onboarding form UI.
 *
 * ARCHITECTURE:
 * - Receives only the minimum user data needed for the form from the Server Component parent.
 * - Server-side auth verification and tenant check have already occurred in onboarding/page.tsx.
 * - All form state, validation, submission, and error handling are client-side here.
 * - The POST to /api/onboarding/institute is still protected server-side by session verification.
 */
...
export interface OnboardingContentProps {
  /** Pre-filled email for the contact field (from session user email) */
  userEmail: string;
}

/**
 * OnboardingContent — renders the institute setup form.
 *
 * Server has already verified: user is authenticated and has no active tenant.
 * This component only handles the form UI and submission to the protected API.
 */
export function OnboardingContent({ userEmail }: OnboardingContentProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (
      formData.slug.trim() &&
      !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(formData.slug.trim().toLowerCase())
    ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Server-side session determines identity and owner tenancy — not the client.
      const payload = {
      ...
      if (response.status === 400) {
        ...
        setGlobalError(
          json.error?.message || 'Invalid onboarding information. Please check form fields.',
        );
        return;
      }
      ...
      <div className="w-full max-w-xl space-y-6">
        ...
        <Card className="shadow-lg">
          <CardHeader className="space-y-1">
            ...
            <CardDescription>
              Create your institute workspace. As the founder, you will receive full owner
              privileges.
            </CardDescription>
          </CardHeader>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <CardFooter className="flex flex-col space-y-3 pt-2">
              ...
              <p className="text-center text-[11px] text-[hsl(var(--muted-foreground))]">
                By completing onboarding, you establish yourself as the primary owner of this
                institute tenant.
              </p>
            </CardFooter>
          </form>
