...
interface ParentAcademicViewsProps {
  ...
  studentName?: string;
  activeView: 'attendance' | 'homework';
}
...
export function ParentAcademicViews({
  ...
  const attendanceQuery = useParentAttendance(studentId);
  const homeworkQuery = useParentHomework(studentId);
      ...
      <Card className="border border-dashed border-[hsl(var(--border))] text-center p-6">
        <CardContent className="space-y-2 pt-2">
          ...
          <p className="text-xs text-[hsl(var(--muted-foreground))] max-w-sm mx-auto">
            Please select an authorized child profile to view academic attendance and homework logs.
          </p>
        </CardContent>
      </Card>
    );
